...
# Constants
THRESHOLD = 1000000  # After how many items should we stop?
P = 8191  # Prime number
...
class CountSketch:
    ...
    def _get_count_sketch(self, index, hash_val):
        return self.count_sketch[index][hash_val]
    ...
    def estimate_frequency(self, elem):
        f_tilde = []
        for j in range(self.d):
            hash_val = self._hash_function(self.ha[j], self.hb[j], elem)
            g = self._generate_sign(elem)
    ...
    def _generate_sign(x):
        random_generator = np.random.RandomState(x)
...
if __name__ == '__main__':
    ...
    for item in true_freq_histogram.keys():
        approximate_freq[item] = count_sketch.estimate_frequency(item)
    ...
    F2_aprox        = sum(val ** 2 for val in approximate_freq.values()) / sigmaR_length ** 2
